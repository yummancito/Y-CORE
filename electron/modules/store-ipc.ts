...
async function fetchDepotKeys(appId: string): Promise<{ depot_id: string; key: string }[]> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (resp.status === 403) {
        const body = (await resp.json().catch(() => ({ error: 'Forbidden' }))) as { error?: string }
      ...
      const data = (await resp.json()) as { depot_keys?: any[] }
  ...
  ipcMain.handle('store:installGame', async (_event, game: StoreGameData) => {
    ...
    try {
      ...
      if (baseDepotResult.addedDepotKeys.length > 0) {
        ...
        actions.push(
          `Added ${baseDepotResult.addedDepotKeys.length} Half-Life base depots`
        )
      }
